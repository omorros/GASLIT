"""Dependency-light smoke tests for critical isolation/data-loss regressions.

These tests avoid provider calls and database access so the automation runner can
lock in request scoping, quarantine audit behavior, and voice idempotency.
"""
...
def test_retrieval_filters_quarantine_in_contract_not_prefilter() -> None:
    source = _read("gaslit/retrieval/librarian.py")

    assert 'prefilter: dict[str, Any] = {"quarantined": False}' not in source
    assert 'prefilter: dict[str, Any] = {}' in source
...
def main() -> int:
    tests = [
        ...
        test_retrieval_filters_quarantine_in_contract_not_prefilter,
