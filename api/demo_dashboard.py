...
def demo_trigger_drift(req: TriggerDriftReq) -> TriggerDriftResp:
        ...
        {"quarantine_id": f"q_demo_{req.memory_id}"},
        {
            ...
            "$set": {
                ...
                "sentinel_explanation": (
                    f"Demo drift injection pushed {req.memory_id} above the "
                    "Sentinel quarantine threshold."
                ),
            },
        },
        upsert=True,
    ...
    return TriggerDriftResp(
        memory_id=req.memory_id,
        inserted=inserted,
        note="Injected retrieval evidence and marked the memory quarantined; poll /api/demo/live for the dossier.",
    )
