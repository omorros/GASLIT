...
def demo_trigger_drift(req: TriggerDriftReq) -> TriggerDriftResp:
    ...
    drift_score = max(0.91, DRIFT_THRESHOLD + 0.01)
    cohort_variance = 6.0
    memory = db[MEMORIES].find_one({"memory_id": req.memory_id}, {"_id": 0}) or {}
    ...
    return TriggerDriftResp(
        memory_id=req.memory_id,
        inserted=inserted,
        note="Appended retrieval evidence and marked the target quarantined; poll /api/memories and /api/sentinel-status.",
    )
