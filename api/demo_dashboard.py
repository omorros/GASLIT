...
def demo_trigger_drift(req: TriggerDriftReq) -> TriggerDriftResp:
    ...
    if not db[QUARANTINE].find_one({"memory_id": req.memory_id}, {"_id": 1}):
        ...
        quarantine_doc = {
            "quarantine_id": f"q_{req.memory_id}_demo_drift",
            "memory_id": req.memory_id,
            "quarantined_at": now,
            "drift_score": DEMO_DRIFT_SCORE,
            "cohort_variance": DEMO_COHORT_VARIANCE,
    ...
    return TriggerDriftResp(
        memory_id=req.memory_id,
        inserted=inserted,
        note="Drift rows appended and quarantine preserved for the scripted demo path.",
    )
