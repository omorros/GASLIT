...
def demo_trigger_drift(req: TriggerDriftReq) -> TriggerDriftResp:
    ...
    memory = db[MEMORIES].find_one(
        {"memory_id": req.memory_id}, {"_id": 0, "user_id": 1}
    )
    if not memory:
    ...
    db[QUARANTINE].update_one(
        {"quarantine_id": f"q_demo_{req.memory_id}"},
        {
            "$setOnInsert": {
                ...
                "responsible_user": memory.get("user_id", "unknown"),
    ...
    return TriggerDriftResp(
        memory_id=req.memory_id,
        inserted=inserted,
        note="Inserted synthetic retrievals without deleting prior evidence; memory is marked quarantined for the demo flow.",
    )
