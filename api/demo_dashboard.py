...
def demo_trigger_drift(req: TriggerDriftReq) -> TriggerDriftResp:
    ...
    # Demo drift is append-only: never delete audit/quarantine state or unquarantine
    # a memory that may already have been blocked by Sentinel.
    db[MEMORIES].update_one(
        {"memory_id": req.memory_id},
        {
            "$inc": {"retrieval_count": inserted},
            "$max": {"drift_score": 0.91, "cohort_variance": 3.2},
        ...
        {"quarantine_id": f"q_demo_{req.memory_id}"},
        {
            ...
            "$set": {
                ...
                "sentinel_explanation": (
                    "Demo drift injection crossed the quarantine threshold for "
                    f"{req.memory_id}."
                ),
            },
        },
