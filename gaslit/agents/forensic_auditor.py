...
def watch_quarantine_stream() -> None:
    """Subscribe to quarantine writes; compose missing dossiers.
    ...
    print("[forensic_auditor] watching quarantine writes...")
    while True:
        try:
            with db[QUARANTINE].watch(pipeline, full_document="updateLookup") as stream:
                for change in stream:
                    doc = change.get("fullDocument") or {}
                    if not doc.get("quarantine_id") or doc.get("dossier_composed_at"):
                        continue
                    try:
                        compose_dossier(doc)
            ...
            print(f"[forensic_auditor] change stream error: {e}; restarting")
            time.sleep(2)
