...
def compose_dossier(quarantine_doc: dict) -> str:
    ...
    if memory is None:
        dossier_text = f"Memory {memory_id} quarantined but no source document was found."
...
def watch_quarantine_stream() -> None:
    """Subscribe to quarantine changes; compose missing forensic dossiers.
        ...
        try:
            with db[QUARANTINE].watch(
                ...
                for change in stream:
                    ...
                    if not doc.get("quarantine_id") or not doc.get("memory_id"):
                        continue
                    if doc.get("dossier_composed_at"):
                        continue
                    try:
                        compose_dossier(doc)
            ...
            print(f"[forensic_auditor] change-stream error; restarting: {type(e).__name__}: {e}")
            time.sleep(2)
