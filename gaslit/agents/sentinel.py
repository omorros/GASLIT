...
def _quarantine_node(state: SentinelState) -> SentinelState:
    ...
    doc = {
        ...
        "siblings_found": [],
        "sentinel_explanation": state.get("nemotron_explanation", ""),
    }
    written = False
    try:
        ...
        # Keep the Sentinel explanation separate from the Forensic dossier.
        # `dossier_text` is owned by the Forensic Auditor and must not be
        # overwritten on duplicate quarantine deliveries.
        if state.get("nemotron_explanation"):
            db[QUARANTINE].update_one(
                {"quarantine_id": qid},
                {"$set": {"sentinel_explanation": state["nemotron_explanation"]}},
            )
