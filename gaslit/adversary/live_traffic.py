    ...
    with httpx.Client(base_url=api_base, timeout=15.0) as client:
        ...
        while time.time() < deadline:
            ...
            payload = {
                "message": q,
                "user_id": "u_2188",
                "thread_id": f"t_traffic_{rng.randint(1000, 9999)}",
                "turn_number": 1,
            }
            try:
                client.post("/api/unprotected-agent", json=payload).raise_for_status()
                client.post("/api/gaslit-agent", json=payload).raise_for_status()
                sent += 1
            except Exception as e:
