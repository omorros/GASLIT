...
def _voice_ids(room: str | None, transcript: str) -> tuple[str, str, int]:
    ...
    digest = hashlib.sha256(f"{r}|{normalized}".encode("utf-8")).hexdigest()
    # Stable per room + transcript so duplicate final STT events are idempotent,
    # while distinct utterances in the same room receive distinct Scribe turns.
    turn_number = int(digest[:8], 16) + 1
